...
def run_inference(
    ...
    for file_path in files:
        ...
        clean_norm, clean_mean, clean_std = ArpesH5Dataset._normalize(clean, return_stats=True)
        noisy_norm, _, _ = ArpesH5Dataset._normalize(noisy, return_stats=True)

        tensor_in = torch.from_numpy(noisy_norm).unsqueeze(0).unsqueeze(0).to(device)
        with torch.no_grad():
            tensor_out = model(tensor_in)
        denoised_norm = tensor_out.squeeze().cpu().numpy()
        denoised = ArpesH5Dataset._denormalize(denoised_norm, clean_mean, clean_std)
        ...
        print(f"[OK] {path.name}: MAE={mae:.4f}, MSE={mse:.4f}")
    ...
    if summaries:
        ...
        with open(csv_path, "w", newline="", encoding="utf-8") as handle:
            writer = csv.DictWriter(
                handle, fieldnames=["file", "model_tag", "dataset", "mae", "mse", "preview"]
            )
            writer.writeheader()
